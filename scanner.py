...
class GUI:
    ...
    def start_scan(self):
        ...
        ports = self.parse_ports(ports_entry)
        ...
        def scan():
            ...
            open_ports = self.open_ports_scan(target, ports_scan)
            services = self.scan_services(target, open_ports)
            host_info = self.get_host_info(target)
    ...
    def parse_ports(self, ports_entry):
        if not ports_entry:
            return None
        ports = []
    ...
    def open_ports_scan(self, target, ports):
        open_ports = []
        for port in ports:
    ...
    def scan_services(self, target, open_ports):
        services = []
